...
function dem_so_binh_luan_san_pham_2()
{
	$id_sp=get_bm("id");
	...
	if($binh_luan_hop_le=="hop_le")
	{
		...
		if($so>4)
		{
			...
			if($tong_so_binh_luan<$so_binh_luan_toi_da)
			{
				...
				if($so_binh_luan_trong_ngay<$so_binh_luan_toi_da_trong_ngay)
				{
					...
					$tv="
						...
						VALUES (NULL, '$noi_dung_binh_luan', 'san_pham', '$id_sp', '$so_lan_xoa','$ngay_hien_tai');
					";
					
					truy_van_bm($tv);
					
					$so=dem_so_binh_luan_san_pham_2();
					...
					$duong_dan="?thamso=chi_tiet_san_pham&id=".$id_sp."&trang=".$st;
					
					if($cach_hien_thi_binh_luan=="moi_truoc_cu_sau")
					{
						$duong_dan="?thamso=chi_tiet_san_pham&id=".$id_sp;
					}
				...
				else 
				{
					...
					$duong_dan="?thamso=chi_tiet_san_pham&id=".$id_sp."&trang=".$trang;
			
					thong_bao_a11("<div style='width:660px' >Có lỗi xảy ra . Web được thiết lập để gửi tối đa ".$so_binh_luan_toi_da_trong_ngay." bình luận trong một ngày . Nếu bạn là người quản lý web thì có thể thay đổi số bình luận tối đa trong ngày trong trang quản lý</div>","Về trang chi tiết sản phẩm",$duong_dan);
			...
			else 
			{
				...
				thong_bao_a11("<div style='width:660px' >Có lỗi xảy ra . Web được thiết lập để gửi tối đa ".$so_binh_luan_toi_da." bình luận . Nếu bạn là người quản lý web thì có thể thay đổi số bình luận tối đa trong trang quản lý</div>","Về trang chi tiết sản phẩm",$duong_dan);
		...
		else 
		{
			...
			thong_bao_a11("<div style='width:660px' >Có lỗi xảy ra . Web được lập trình 4 giây mới thêm được bình luận , có thể ai đó đã thêm bình luận nào đó vào khoảng 4 giây gần đây </div>","Về trang chi tiết sản phẩm",$duong_dan);
